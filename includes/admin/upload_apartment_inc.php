<?php
    ...
    if(isset($_POST['add_apartment_btn'])) {
        ...
        if(empty($apartment_title) || empty($apartment_price) || empty($number_rooms) || empty($apartment_des) || empty($number_bed_rooms) || empty($apartment_status)) {
            ...
            $insert_apartment = "INSERT INTO `apartments` (`apartment_title`,`apartment_price`, `apartment_des`, `number_rooms`, `number_bedrooms`, `status`, `images`) VALUES(?,?,?,?,?,?,?)";

            $stmt = $connect->prepare($insert_apartment);
 
            $stmt->bind_param('sssssss' , $apartment_title, $apartment_price, $apartment_des,  $number_rooms, $number_bed_rooms, $apartment_status, $files_array);
